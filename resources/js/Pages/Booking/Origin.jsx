...
const OriginPage = () => {
    ...
    const mappedLocation = locations.filter(
        (item) =>
            item.city.toLocaleLowerCase().search(search.toLocaleLowerCase()) !=
                -1 ||
            item.station
                .toLocaleLowerCase()
    ...
    return (
        <BookingLayout>
            <header>
                <div className="flex flex-row items-center justify-between w-full min-h-full pb-2 pt-8 bg-[#49ABFF] px-4 rounded-b-md">
                    <Link href="/booking">
                        <svg
                            width="46"
                            height="46"
                    ...
                    <div className="relative w-full">
                        ...
                        <input
                            ...
                            className="font-[Poppins] w-full mr-3 pl-10 rounded-xl p-2 outline-none border-[#C4C4C4]"
                        />
                    </div>
                </div>
            </header>
                ...
                {mappedLocation.length < 1 ? (
                    ...
                    mappedLocation.map((item, index) => (
                        <InputRow key={index}>
                            ...
                            >
                                <h3 className="font-light text-[14px] font-[Poppins]">
                                    {item.city}
                                </h3>
                                <h3 className="font-medium text-[16px] font-[Poppins]">
                                    {item.station}
                                </h3>
                            </div>
                        </InputRow>
